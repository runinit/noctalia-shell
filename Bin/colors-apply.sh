...
fuzzel)
    ...
    # Read font settings from noctalia config
    FONT_NAME="Inter"
    FONT_SIZE="11"
    if [ -f "$SETTINGS_FILE" ]; then
        # Extract font name and size from settings.json
        FONT_NAME=$(grep -Po '"fontDefault":\s*"\K[^"]+' "$SETTINGS_FILE" || echo "Inter")
        FONT_SIZE=$(grep -Po '"fontDefaultSize":\s*\K[0-9]+' "$SETTINGS_FILE" || echo "11")
    fi
...
EOF
        ...
        if grep -q '^font=' "$CONFIG_FILE"; then
            sed -i "s|^font=.*|font=${FONT_NAME}:size=${FONT_SIZE}|" "$CONFIG_FILE"
        else
            # Add font line in [main] section if it doesn't exist
            if grep -q '^\[main\]' "$CONFIG_FILE"; then
                sed -i "/^\[main\]/a font=${FONT_NAME}:size=${FONT_SIZE}" "$CONFIG_FILE"
            fi
        fi

        # Remove any existing theme include line.
        sed -i '/include=.*themes.*noctalia/d' "$CONFIG_FILE"

        # Add the include line at the end of the [main] section
        # This finds the [main] section and adds the include before the next section starts
        if grep -q '^\[main\]' "$CONFIG_FILE"; then
            # Insert after the last line of [main] section (before next section or colors)
            sed -i '/^\[main\]/,/^\[/ { /^\[main\]/! { /^\[/i include=~/.config/fuzzel/themes/noctalia
            }}' "$CONFIG_FILE"
        else
            # If no [main] section exists, add it at the top with include
            sed -i "1i [main]\\nfont=${FONT_NAME}:size=${FONT_SIZE}\\ninclude=~/.config/fuzzel/themes/noctalia\\n" "$CONFIG_FILE"
        fi

        echo "Applied font: ${FONT_NAME}:size=${FONT_SIZE}"
    fi
    ;;
